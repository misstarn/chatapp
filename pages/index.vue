...
const changeInput = () => {
    ...
    // 查找消息人
    friends.value = fri.filter((friend) => friend.name.indexOf(input.value) !== -1)
}
...
const changeInput2 = () => {
    ...
    // console.log(userGroups)
    users2.value = userGroups.filter(userGroup => userGroup.user.name.indexOf(input2.value) !== -1)

    // 搜索群
    groups2.value = groups.value.filter(group => group.name.indexOf(input2.value) !== -1)
}
...
<template>
    <v-app>
        <div class="p-4 lg:p-8 max-w-[1200px] w-full mx-auto">
            ...
            <v-navigation-drawer v-model="drawer" class="px-2" width="350">
                <div class="flex items-center mt-6">
                    <!-- 个人头像，点击打开个人信息 -->
                    <v-list-item>
                        <v-menu :offset="menuoffset" transition="slide-y-transition" :close-on-content-click="false"
                            location="end">
                    ...
                    <div>
                        ...
                        <v-sheet>
                            <div class="px-2 pt-8">
                                ...
                                <v-text-field :loading="loading" density="compact" variant="outlined" label="搜索"
                                    prepend-inner-icon="mdi-magnify" single-line hide-details v-model="input" color="info"
                                    @update:modelValue="changeInput" clearable @click:clear="clear"></v-text-field>
                                <!-- 选项 Recent Chats-->
                        ...
                        <div class="relative overflow-y-auto h-[500px] mt-4" max-height="500">
                            <v-list @click:select="select" nav>
                                <v-list-item v-for="user in friends" :key="user.usernam" :value="user" active-class="active"
                        ...
                        <div>
                            ...
                            <v-sheet>
                                <div class="px-2 pt-8">
                                    ...
                                    <div class="flex space-x-2">
                                        <v-text-field :loading="loading" density="compact" variant="outlined" label="搜索"
                                            prepend-inner-icon="mdi-magnify" single-line hide-details v-model="input2"
                                            @update:modelValue="changeInput2" color="info" clearable></v-text-field>
                                        ...
                                        <v-menu location="end" transition="slide-x-transition">
                                            ...
                                            <v-list v-list nav density="compact">
                                                <v-list-item
                                                    @click="() => { overlayGroup = true; createGroupData.friends = [] }">
                                                    发起群聊
                                                </v-list-item>
                            ...
                            <v-window v-model="tab" class="px-2">
                                <v-window-item value="friends">
                                    <div class="relative overflow-y-auto h-auto mt-4" max-height="500">
                ...
                <v-card class="min-w-[350px] relative">
                    ...
                    <v-card-text>
                        <v-text-field :loading="loading" density="compact" variant="outlined" label="Search templates"
                            append-inner-icon="mdi-magnify" single-line hide-details @click:append-inner="searchUserOrGroup"
                            color="info" @keydown.enter="searchUserOrGroup" v-model="searchKey"></v-text-field>
                        ...
                        <v-sheet class="h-[399px] overflow-auto mt-2">
                            ...
                            <v-list v-if="userOrGroup.users">
                                ...
                                <v-list-item class="px-2 py-2" v-for="(item, i) in userOrGroup.users" :key="item.id"
                                    ...
                                    <v-list-item-title style="font-size: 14px" v-text="item.name"></v-list-item-title>
                            ...
                            <v-list v-if="userOrGroup.groups">
                                ...
                                <v-list-item class="px-2 py-2" v-for="(item, i) in userOrGroup.groups" :key="item.id"
                                    :value="item">
                                    <template v-slot:prepend>
                                        <v-avatar size="40px">
                    ...
                    <v-card-text>
                        <v-list>
                            <v-list-item class="px-2">
                                ...
                                <v-list-item-title style="font-size: 13px;" v-text="sendItem.name"></v-list-item-title>
                        ...
                        <v-sheet class="px-1">
                            ...
                            <v-textarea variant="outlined" hide-details density="compact" color="info" single-line
                                autocomplete v-model="applyForm.message"></v-textarea>
                            <div class="text-xs my-2 mt-4 pl-2">分组</div>
                            <div class="flex gap-2">
                                <v-select single-line label="Select" density="compact" hide-details color="info"
                                    v-model="applyForm.userGroup" :items="selectgroups" item-title="name" item-value="id"
                ...
                <v-card class="min-w-[300px] pb-4">
                    ...
                    <v-card-text>
                        ...
                        <v-sheet class="px-1">
                            <div class="text-xs mb-2 pl-2">填写验证信息</div>
                            <v-textarea variant="outlined" hide-details density="compact" color="info" single-line
                                autocomplete v-model="applyGroupFrom.message"></v-textarea>
                        </v-sheet>
                    </v-card-text>
                    <v-card-actions class="pr-5">
                        <v-spacer></v-spacer>
                ...
                <v-card class="min-w-[350px] relative">
                    ...
                    <v-card-text>
                        <v-sheet class="px-1">
                            ...
                            <v-text-field :loading="loading" density="compact" variant="outlined" label="群聊名称" single-line
                                hide-details @click:append-inner="searchUserOrGroup" color="info"
            ...
            <v-card>
                <div class="flex">
                    ...
                    <div class="hidden lg:flex">
                                ...
                                <v-menu :offset="menuoffset" transition="slide-y-transition" :close-on-content-click="false"
                                    ...
                                    <v-card>
                                        <v-sheet>
                                            ...
                                            <v-list :lines="false">
                                                <v-list-item class="py-4" v-for="(item, i) in profile" :key="i"
                                                    :value="item">
                                                    <template v-slot:prepend>
                                                        <v-avatar class="bg-lightprimary" style="background-color: #ecf2ff;"
                                                            rounded="sm">
                                            ...
                                            <div class="px-4 py-3">
                                                <v-btn @click="logout" block class="text-none text-subtitle-1"
                        ...
                        <!-- 消息列表和好友列表 -->
                        <div class="overflow-auto border-l border-r border-r-[rgb(229,234,239)] flex-shrink-0">
                            <div v-if="msg" class="flex flex-1 w-full">
                                <!-- 左侧消息列表 -->
                                <div class=" min-h-[500px] w-[280px] lg:w-[350px] transition-all duration-100">
                                    <div>
                                        <!-- 搜索 -->
                                        <v-sheet>
                                            <div class="px-4 pt-8">
                                                ...
                                                <v-menu transition="slide-y-transition">
                                                    <template v-slot:activator="{ props }">
                                                        <v-btn v-bind="props" variant="plian"
                                        ...
                                        <div class="relative overflow-y-auto h-[500px] mt-4" max-height="500">
                                            <v-list @click:select="select" class="px-4" nav>
                                                <v-list-item v-for="user in friends" :key="user.usernam" :value="user"
                                                    ...
                                                    <template v-slot:prepend>
                                                        <!-- 好友 -->
                                                        <v-badge v-if="user.tab == 'friends'" dot
                                                            :color="user.online ? 'success' : ''" offset-y="32">
                                                            <v-avatar size="45">
                                                                <v-img v-if="user.avatar.url" cover alt="Avatar"
                                                                    :src="baseUrl + user.avatar.url"></v-img>
                                                                <v-icon v-else></v-icon>
                                                        ...
                                                        <div class="flex items-center mt-1">
                                                            ...
                                                            <template v-if="user.tab == 'groups'">
                                                                <h6 class="text-xs font-normal"
                                                                    ...
                                                                    v-if="user.lastMsg">
                                                                    {{
                                                                        user.lastMsg.username }}：{{ user.lastMsg.type
        == 'image' ? `[图片]` :
        user.lastMsg.content }}</h6>
                                                            </template>
                                                            <!-- 未读消息数 -->
                            ...
                            <div v-else class="flex flex-1 w-full relative">
                                <!-- left 联系人/群列表 -->
                                <div class=" min-h-[500px] w-[350px] transition-all duration-100">
                                    <div>
                                        <!-- 搜索 / 列表切换 -->
                                        <v-sheet>
                                            <div class="px-4 pt-8">
                                                ...
                                                <div class="flex space-x-2">
                                                    <v-text-field :loading="loading" density="compact" variant="outlined"
                                                        label="搜索" prepend-inner-icon="mdi-magnify" single-line hide-details
                                                        v-model="input2" @update:modelValue="changeInput2" clearable
                                                        color="info"></v-text-field>

                                                    <!-- 添加好友 -->
                                                    <v-menu location="end" transition="slide-x-transition">
                                                        <template v-slot:activator="{ props }">
                                                            <v-btn v-bind="props" icon="mdi-plus" variant="tonal"
                                                                rounded="lg"></v-btn>
                                                        </template>
                                                        <v-list v-list nav density="compact">
                                                            <v-list-item
                                        ...
                                        <v-window v-model="tab" class="px-4" v-if="!input2">
                                            <v-window-item value="friends">
                                                <div class="relative overflow-y-auto h-[500px] mt-4" max-height="500">
                                                    <v-expansion-panels multiple variant="accordion" class="expan"
                                                        v-model="panel">
                                                        <v-expansion-panel v-for="userGroup in userGroupMembers"
                                                            :key="userGroup.id">
                                                            <v-expansion-panel-title>{{ userGroup.name }}({{
        ...
        userGroup.user_group_members.length
                                                            ...
                                                            <v-expansion-panel-text>
                                                                <v-list @click:select="selectDetail" class="pt-2 pb-0" nav>
                                                                    <v-list-item
    ...
    user.user.description }}</h6>
                    ...
                    <div class="flex-1 w-full relative">
                        <div class="lg:hidden">
                            <v-btn @click="drawer = !drawer" class="w-full lg:hidden" variant="text" rounded="0">
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-menu-2 mr-2"
                                ...
                                <v-sheet>
                                        ...
                                        <div class="flex items-center gap-3 py-4 justify-between">
                                            <div class="flex gap-4 items-center">
                                                ...
                                                <div class="flex flex-col justify-center">
                                                    ...
                                                    <small class="mt-1 text-sm">UID: {{ detail.tab == 'groups' ?
                                                        detail.groupname :
                                            ...
                                            <div class="flex items-center space-x-2">
                                                ...
                                                <!-- 发起聊天 -->
                                                <v-btn @click="chat" variant="tonal"
                                                    icon="mdi-message-reply-outline"></v-btn>
                                            </div>
                                        </div>
                                        <hr>
                                    </v-sheet>
                                    ...
                                    <v-sheet v-if="detail.tab === 'friends'" class="px-8 py-4">
                                        <div class="divide-x flex space-x-2">
                                            ...
                                            <span v-if="detail.birthday" class="pl-2">{{ format(new Date(detail.birthday),
                                                'M月d日')
                                        ...
                                        <v-list class="px-0 mt-8" density="compact" nav>
                                            <v-list-item class="px-0 py-2">
                                                ...
                                                <template v-slot:append>
                                                    <v-select density="compact" single-line hide-details :items="['好友']"
                                        ...
                                        <v-list lines="two" variant="tonal" v-if="notes" class="pt-0 px-4" nav>
                                            <v-list-item v-for="user in notes" class="my-4">
                                                ...
                                                <template v-slot:title>
                                                    <div>
                                                        ...
                                                        <template v-if="user.me">
                                                            ...
                                                            <span
                                                                v-else-if="user.message.status == 'accepted'">同意了你的邀请</span>
                                                ...
                                                <template v-slot:append>
                                                            ...
                                                            <v-card-text>
                                                                <v-list>
                                                                    <v-list-item>
                                                                        ...
                                                                        <template v-slot:subtitle>
                                                                            <div class="mt-1">留言：{{ user.message.content }}
                                                                            </div>
                                                                        </template>
                                                                    </v-list-item>
                                                                </v-list>
                                                    ...
                                                    <div v-else class="text-sm mr-4">{{ user.message.status == 'pending' ?
                                                        '等待验证'
                                        ...
                                        <v-list lines="two" variant="tonal" v-if="notes" class="pt-0 px-4" nav>
                                            <v-list-item v-for="note in gnotes" class="my-4">
                                                ...
                                                <template v-slot:title>
                                                    <div>
                                                        <template v-if="note.me">
                                                            <span class="text-info mr-2">{{ note.message.group.name
                                                            }}</span>
                                                            <span v-if="note.message.status == 'pending'">正在验证你的入群申请</span>
                                                            <span
                                                                v-else-if="note.message.status == 'accepted'">同意了你的入群申请</span>
                                                ...
                                                <template v-slot:append>
                                                            ...
                                                            <v-card-text>
                                                                <v-list>
                                                                    <v-list-item>
                                                                        ...
                                                                        <template v-slot:subtitle>
                                                                            <div class="mt-1">留言：{{ note.message.content }}
                                                                            </div>
                                                                        </template>
                                                                    </v-list-item>
                                                                </v-list>
                                                    ...
                                                    <div v-else class="text-sm mr-4">{{ note.message.status == 'pending' ?
                                                        '等待验证'
